// The panel's own scripts. Moved out of _scripts.html on 2026-09-23: 2,560
// lines and 119 KB inside a template every panel page includes, and _base.html
// sends those pages Cache-Control: no-cache - so all of it crossed the wire
// again on every load.
//
// The nineteen url_for() calls it carried stay in the template, as the small
// DDC_PANEL_URLS object right above the tag that loads this file. That is the
// pattern tasks/list.html already used for window.DDC_CONFIG.urls: the routes
// are Flask's to name, the code is not.
...
// --- Global variables and functions for Unsaved Changes (moved out of DOMContentLoaded) ---
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (floatingDiscardButton) {
            floatingDiscardButton.addEventListener('click', function() {
                if (confirm(t('config.confirm_discard'))) {
        ...
        // Enter in a text field (or a button without type) would submit the form natively as
        // POST / -> 405 page, unsaved edits lost and the checks in saveConfigAjax() skipped.
        if (configForm) {
        ...
        if (configForm) {
            ...
            criticalElements.forEach(element => {
                ...
                function updateRestartBadge(el) {
                    const badge = el.closest('.mb-3').querySelector('.restart-badge');
                    if (badge) {
                        const changed = el.value !== el.getAttribute('data-initial-value');
                        badge.style.display = changed ? '' : 'none';
                    }
                }

                element.addEventListener('input', () => {
                    console.log(`INPUT event on ${element.name || element.id}: ${element.value}`);
                    updateRestartBadge(element);
        ...
        window.rawLogsCache = ''; // Cache for the full logs (global for LogAutoRefresh class)
        ...
        window.displayFilteredLogs = function displayFilteredLogs() {
            if (!logContentElement) return;
            const filteredLogsText = filterLogs(window.rawLogsCache, currentLogLevel);
        ...
        if (refreshDockerButton) {
            refreshDockerButton.addEventListener('click', function() {
                ...
                fetch(DDC_PANEL_URLS.refreshContainers, {
                    method: 'POST',
    ...
    function collectChannelIdInputs(prefix) {
        const entries = [];
        document.querySelectorAll(`input[name^="${prefix}_channel_id_"]`).forEach(input => {
            const value = (input.value || '').trim();
            if (value) {
                entries.push({ value: value, input: input });
            }
        });
        return entries;
    }

    // A channel ID may appear only once across both permission tables. The backend parses the
    // status rows first and the control rows second, so a duplicate silently becomes a control
    // channel and /serverstatus is denied there. Marks the offending inputs and returns the
    // duplicated IDs.
    function validateChannelIdUniqueness() {
        const entries = collectChannelIdInputs('status').concat(collectChannelIdInputs('control'));
        const seen = {};
        const duplicates = [];
        ...
        entries.forEach(entry => {
            ...
            if (seen[entry.value]) {
                if (duplicates.indexOf(entry.value) === -1) {
    ...
    // Re-check while typing so the red marking disappears as soon as the conflict is resolved.
    // Delegated because channel rows are added dynamically.
    document.addEventListener('input', function(event) {
        const name = event.target && event.target.name;
        if (name && (name.indexOf('status_channel_id_') === 0 || name.indexOf('control_channel_id_') === 0)) {
            validateChannelIdUniqueness();
        }
    });
    ...
    function saveConfigAjax() {
        ...
        // Block the save while the same channel ID is used twice - it would be stored as a
        // control channel and /serverstatus would be refused in it.
        const duplicateChannelIds = validateChannelIdUniqueness();
        if (duplicateChannelIds.length > 0) {
            notification.textContent = t('permissions.duplicate_channel_ids', { ids: duplicateChannelIds.join(', ') });
        ...
        formElements.forEach(element => {
            ...
            // The CSRF field added by _base.html travels as X-CSRFToken header; it must not
            // end up in the saved configuration.
        ...
        const uiLanguageField = document.getElementById('ui_language');
        if (uiLanguageField) {
            formData.set('ui_language', uiLanguageField.value);
        }
        ...
        fetch(DDC_PANEL_URLS.saveConfig, {
            method: 'POST',
            body: formData,
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
        .then(response => { 
            if (!response.ok) {
                 // Second then() argument: only a JSON parse failure falls back to the status code,
                 // the server's message (e.g. an expired CSRF session) is kept.
                 return response.json()
                     .then(errData => {
                         // ddcServerErrorMessage (_base.html) translates a rejected CSRF check
                         const message = (typeof window.ddcServerErrorMessage === 'function')
                             ? window.ddcServerErrorMessage(errData, `HTTP error ${response.status}`)
                             : (errData.message || errData.error || `HTTP error ${response.status}`);
    ...
    async function fetchLogsByType(logType) {
        ...
        try {
            ...
            switch(logType) {
                case 'container':
                    endpoint = DDC_PANEL_URLS.containerLogs;
                    break;
                case 'bot':
                    endpoint = DDC_PANEL_URLS.botLogs;
                    break;
                case 'discord':
                    endpoint = DDC_PANEL_URLS.discordLogs;
                    break;
                case 'webui':
                    endpoint = DDC_PANEL_URLS.webuiLogs;
                    break;
                case 'application':
                    endpoint = DDC_PANEL_URLS.applicationLogs;
                    break;
                case 'actions':
                    endpoint = DDC_PANEL_URLS.actionLogs;
                    break;
                default:
                    endpoint = DDC_PANEL_URLS.containerLogs;
            }
    ...
    function refreshTempDebugStatus() {
        fetch(DDC_PANEL_URLS.tempDebugStatus, {
            method: 'GET',
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
        .then(response => response.json())
    ...
    function enableTempDebug() {
        ...
        fetch(DDC_PANEL_URLS.enableTempDebug, {
            method: 'POST',
            body: formData,
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
    ...
    function disableTempDebug() {
        ...
        fetch(DDC_PANEL_URLS.disableTempDebug, {
            method: 'POST',
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        })
        .then(response => response.json())
    ...
    class LogAutoRefresh {
        ...
        async smartRefreshContainerLogs() {
            ...
            try {
                // Use the current log type endpoint (same as manual fetch)
                const logType = window.currentLogType || 'container';
                const endpoints = {
                    'container': DDC_PANEL_URLS.containerLogs,
                    'bot': DDC_PANEL_URLS.botLogs,
                    'discord': DDC_PANEL_URLS.discordLogs,
                    'webui': DDC_PANEL_URLS.webuiLogs,
                    'application': DDC_PANEL_URLS.applicationLogs,
                    'actions': DDC_PANEL_URLS.actionLogs
                };
                const endpoint = endpoints[logType] || endpoints['container'];
                ...
                if (newHash !== this.lastContainerLogHash) {
                    ...
                    window.rawLogsCache = newContent;
                    if (typeof window.displayFilteredLogs === 'function') {
                        window.displayFilteredLogs();
                    } else {
                        logContentElement.textContent = newContent;
                    }
