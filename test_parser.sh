#!/bin/bash
# Test if the form parser works correctly

echo "Creating test script in config directory..."

cat > /mnt/user/appdata/dockerdiscordcontrol/config/test_parse_form.py << 'ENDPYTHON'
#!/usr/bin/env python3
import sys
...
docker exec dockerdiscordcontrol python3 /app/config/test_parse_form.py

echo ""
echo "Cleaning up..."
rm /mnt/user/appdata/dockerdiscordcontrol/config/test_parse_form.py
