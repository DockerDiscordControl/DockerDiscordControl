...
def test_a_group_checkbox_carries_the_prefix_the_rule_needs():
    ...
    assert 'value="group:' in JS, "a ticked group would be saved as a container name"
...
def test_reopening_a_rule_ticks_the_group_again():
    """The tick comes from the same list as the containers, so it must include them."""
    after_lookup = JS.split("const containerCheckboxes")[1][:200]

    assert "aas-group-checkbox" in after_lookup, "a saved group rule opens with nothing ticked"
